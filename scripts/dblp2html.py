...
Also rewrites the "Selected recent papers" block in index.html, between the
`<!-- BEGIN:selected-papers -->` / `<!-- END:selected-papers -->` markers.
Selection rule (2026-08-30, revised 2026-08-30): candidates are papers from
the last ~24 months — approximated as the current calendar year and the two
before it, since DBLP only carries a year (no month) for most venue papers,
so an exact rolling 24-month cutoff isn't computable from this data. Ranked
by recency first, then by venue preference (STOC, SODA, PODC, ICALP, ESA,
everything else) as a tiebreaker, and capped at the top 5. The window and
ranking both move with the clock, so this needs no yearly edit.

Re-run this instead of hand-editing publications.html or the selected-papers
block in index.html.
"""
...
VENUE_PREFERENCE = ['STOC', 'SODA', 'PODC', 'ICALP', 'ESA']
SELECTED_WINDOW_YEARS = 2   # candidate pool: this year and this many before it
SELECTED_MAX = 5

INDEX_START = '<!-- BEGIN:selected-papers -->'
INDEX_END = '<!-- END:selected-papers -->'
...
def clean_name(raw):
    ...
    return html.escape(re.sub(r'\s+\d{4}$', '', raw or ''))
...
def authors_excl_self_of(pub, my_pid):
    """Co-author list for the homepage's 'with ...' line — self omitted, since
    the page is already about him."""
    out = [clean_name(a.text) for a in pub.findall('author') if a.get('pid') != my_pid]
    if len(out) > 1:
        return ', '.join(out[:-1]) + ' and ' + out[-1]
    return out[0] if out else ''
...
def selected_papers_block(entries):
    this_year = datetime.date.today().year
    cutoff = this_year - SELECTED_WINDOW_YEARS
    eligible = [e for e in entries if e['year'] >= cutoff]

    def rank(e):
        try:
            return VENUE_PREFERENCE.index(e['venue'])
        except ValueError:
            return len(VENUE_PREFERENCE)

    eligible.sort(key=lambda e: (-e['year'], rank(e), e['title']))
    eligible = eligible[:SELECTED_MAX]

    rows = ['<ul>']
    for e in eligible:
        title = html.escape(e['title'])
        head = '<a href="%s">%s</a>' % (html.escape(e['url']), title) if e['url'] else title
        line = '  <li>%s<span class="venue">%s&nbsp;%s</span>' % (head, html.escape(e['venue']), e['year'])
        if e['arxiv']:
            line += ' <a class="arxiv" href="https://arxiv.org/abs/%s">arXiv</a>' % e['arxiv']
        if e['authors_excl_self']:
            line += '<br>\n      <span class="meta">with %s</span>' % e['authors_excl_self']
        rows.append(line + '</li>')
    rows.append('</ul>')
    return '\n'.join(rows), len(eligible)


def update_index(entries, index_path):
    block, n = selected_papers_block(entries)
    text = Path(index_path).read_text(encoding='utf-8')
    pattern = re.compile(re.escape(INDEX_START) + r'.*?' + re.escape(INDEX_END), re.S)
    if not pattern.search(text):
        sys.exit('%s: markers %s / %s not found' % (index_path, INDEX_START, INDEX_END))
    replacement = '%s\n<!-- generated by scripts/dblp2html.py — do not hand-edit between these markers -->\n%s\n%s' % (
        INDEX_START, block, INDEX_END)
    text = pattern.sub(lambda _: replacement, text)
    Path(index_path).write_text(text, encoding='utf-8')
    return n
...
def main():
    ...
    ap.add_argument('--index', default='index.html',
                    help='homepage whose selected-papers block gets rewritten '
    ...
    for pub in published:
        ...
        entries.append({
            ...
            'authors_excl_self': authors_excl_self_of(pub, args.pid),
    ...
    if args.index:
        n = update_index(entries, args.index)
        print('Updated %s — %d selected papers (last ~24mo, recency then %s preferred).' %
              (args.index, n, ', '.join(VENUE_PREFERENCE)))
